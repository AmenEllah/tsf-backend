...
export class MonthlyNetIncomeComponent implements OnInit, OnDestroy {
  ...
  loadPage(page?: number, dontNavigate?: boolean): void {
    ...
    this.monthlyNetIncomeService
      ...
      .subscribe(
        (res: HttpResponse<IMonthlyNetIncome[]>) => this.onSuccess(res.body, res.headers, pageToLoad, !dontNavigate),
        () => this.onError()
      );
  }
  ...
  registerChangeInMonthlyNetIncomes(): void {
    this.eventSubscriber = this.eventManager.subscribe('monthlyNetIncomeListModification', () => this.loadPage());
  }
  ...
  protected onSuccess(data: IMonthlyNetIncome[] | null, headers: HttpHeaders, page: number, navigate: boolean): void {
    this.totalItems = Number(headers.get('X-Total-Count'));
    this.page = page;
    if (navigate) {
      this.router.navigate(['/monthly-net-income'], {
        queryParams: {
          page: this.page,
          size: this.itemsPerPage,
