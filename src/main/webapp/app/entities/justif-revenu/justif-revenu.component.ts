...
export class JustifRevenuComponent implements OnInit, OnDestroy {
  ...
  loadPage(page?: number, dontNavigate?: boolean): void {
    ...
    this.justifRevenuService
      ...
      .subscribe(
        (res: HttpResponse<IJustifRevenu[]>) => this.onSuccess(res.body, res.headers, pageToLoad, !dontNavigate),
        () => this.onError()
      );
  }
  ...
  registerChangeInJustifRevenus(): void {
    this.eventSubscriber = this.eventManager.subscribe('justifRevenuListModification', () => this.loadPage());
  }
  ...
  protected onSuccess(data: IJustifRevenu[] | null, headers: HttpHeaders, page: number, navigate: boolean): void {
    this.totalItems = Number(headers.get('X-Total-Count'));
    this.page = page;
    if (navigate) {
      this.router.navigate(['/justif-revenu'], {
        queryParams: {
          page: this.page,
          size: this.itemsPerPage,
