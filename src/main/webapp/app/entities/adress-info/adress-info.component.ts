...
export class AdressInfoComponent implements OnInit, OnDestroy {
  ...
  loadPage(page?: number, dontNavigate?: boolean): void {
    ...
    this.adressInfoService
      ...
      .subscribe(
        (res: HttpResponse<IAdressInfo[]>) => this.onSuccess(res.body, res.headers, pageToLoad, !dontNavigate),
        () => this.onError()
      );
  }
  ...
  registerChangeInAdressInfos(): void {
    this.eventSubscriber = this.eventManager.subscribe('adressInfoListModification', () => this.loadPage());
  }
  ...
  protected onSuccess(data: IAdressInfo[] | null, headers: HttpHeaders, page: number, navigate: boolean): void {
    this.totalItems = Number(headers.get('X-Total-Count'));
    this.page = page;
    if (navigate) {
      this.router.navigate(['/adress-info'], {
        queryParams: {
          page: this.page,
          size: this.itemsPerPage,
