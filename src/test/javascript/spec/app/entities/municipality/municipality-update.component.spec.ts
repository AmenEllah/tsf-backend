import { ComponentFixture, TestBed, fakeAsync, tick } from '@angular/core/testing';
import { HttpResponse } from '@angular/common/http';
import { FormBuilder } from '@angular/forms';
import { of } from 'rxjs';

import { TfsBackendTestModule } from '../../../test.module';
import { MunicipalityUpdateComponent } from 'app/entities/municipality/municipality-update.component';
import { MunicipalityService } from 'app/entities/municipality/municipality.service';
import { Municipality } from 'app/shared/model/municipality.model';

describe('Component Tests', () => {
  describe('Municipality Management Update Component', () => {
    let comp: MunicipalityUpdateComponent;
    let fixture: ComponentFixture<MunicipalityUpdateComponent>;
    let service: MunicipalityService;

    beforeEach(() => {
      TestBed.configureTestingModule({
        imports: [TfsBackendTestModule],
        declarations: [MunicipalityUpdateComponent],
        providers: [FormBuilder],
      })
        .overrideTemplate(MunicipalityUpdateComponent, '')
        .compileComponents();

      fixture = TestBed.createComponent(MunicipalityUpdateComponent);
      comp = fixture.componentInstance;
      service = fixture.debugElement.injector.get(MunicipalityService);
    });

    describe('save', () => {
      it('Should call update service on save for existing entity', fakeAsync(() => {
        // GIVEN
        const entity = new Municipality(123);
        spyOn(service, 'update').and.returnValue(of(new HttpResponse({ body: entity })));
        comp.updateForm(entity);
        // WHEN
        comp.save();
        tick(); // simulate async

        // THEN
        expect(service.update).toHaveBeenCalledWith(entity);
        expect(comp.isSaving).toEqual(false);
      }));

      it('Should call create service on save for new entity', fakeAsync(() => {
        // GIVEN
        const entity = new Municipality();
        spyOn(service, 'create').and.returnValue(of(new HttpResponse({ body: entity })));
        comp.updateForm(entity);
        // WHEN
        comp.save();
        tick(); // simulate async

        // THEN
        expect(service.create).toHaveBeenCalledWith(entity);
        expect(comp.isSaving).toEqual(false);
      }));
    });
  });
});
